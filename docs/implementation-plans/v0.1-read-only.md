...
- Write paths: `features create / edit / comment`, `requirements edit /
  comment`, `todos create / edit / done / reopen`. Designed and shipped
  per [`write-commands.md`](write-commands.md).
...
- **`CONTRIBUTING.md`** — `cargo test` / `cargo insta review` /
  ...
  recipe.
- **`docs/implementation-plans/`** — this directory. One file per
  scoped initiative (v0.1 read-only surface, OAuth login, write
  commands). Update each file when we change direction; don't let
  them rot.
